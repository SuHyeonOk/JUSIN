...
Engine::_vec3 Engine::CCalculator::PickingOnTerrain(HWND hWnd, const CTerrainTex* pTerrainBufferCom, const CTransform* pTerrainTransformCom)
{
	...
	GetCursorPos(&ptMouse); // 마우스 얻어오기
	ScreenToClient(hWnd, &ptMouse); // 스크린 좌표로 바꿔주기

	_vec3		vPoint; // 포인터 구조체 이기 때문에

	D3DVIEWPORT9		ViewPort; // 뷰 포트 구조체
	ZeroMemory(&ViewPort, sizeof(D3DVIEWPORT9));
	m_pGraphicDev->GetViewport(&ViewPort); // WINCX Width, Height이 담김
