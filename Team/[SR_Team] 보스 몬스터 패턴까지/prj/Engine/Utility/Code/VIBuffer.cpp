...
{
	// CreateVertexBuffer : 매개변수로 받은 여러 속성들을 가진 버텍스를 생성한다.
...
void CVIBuffer::Render_Buffer(void)
{
	...
	// 0의 의미 : 스트림 번호
	// 내가 여러 텍스쳐를 가지고 있다면 인덱스를 매겨줄 수 있다.
	// 한번에 모든 텍스쳐를 다 그릴 수 없다.
	// 기본적으로 4개의 렌더링 슬롯을 제공하는데 디폴트로 0을 씀.
	// 따라서, 텍스쳐 출력할 때 어떤 슬롯을 가지고 렌더링 할 것인지 지정할 수 있음.
