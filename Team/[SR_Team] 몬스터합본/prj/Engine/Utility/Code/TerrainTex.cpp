...
// dwVtxItv : 정점과 정점 사이의 간격이 필요할 수도 있다.
// 현재는 1(클수록 정교함이 떨어짐)
// 지형을 만들면서 인터벌에 대한 고민이 필요하다.
{
	// 텍스쳐를 만드는 방법
	/*
	...
	*(((_ulong*)LockRect.pBits) + iIndex) = D3DCOLOR_ARGB(255, 255, 0, 0);
	...
	for (_ulong i = 0; i < dwCntZ; ++i)
	{
		for (_ulong j = 0; j < dwCntX; ++j)
		{
			...
			pVertex[dwIndex].vNormal = { 0.f, 0.f, 0.f }; // 법선벡터 초기화


			pVertex[dwIndex].vTexUV = { _float(j) / (dwCntX - 1) * 20.f ,
										_float(i) / (dwCntZ - 1) * 20.f };
					// 디테일 맵?
					// 내부적인 옵션들이 다양해, 어떻게 출력하느냐에 대한 옵션도 있다.
					// 텍스쳐UV는 0에서 1로 출력되는데 *20을 해주면서 1을 넘는 범위가 발생함
					// 그러면 그 옆을 0~1로 다시 출력한다. 즉, 가로 세로 20장의 연속적인 출력구조를 갖게됨
					// => 디폴트 샘플링 옵션
					// 쉐이더에 따라 미러~옵션을 쓰면 1을 넘어갈 때 반전된 이미지로 반복한다.
		}
	}
