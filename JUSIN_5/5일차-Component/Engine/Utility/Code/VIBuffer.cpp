...
{
	// CreateVertexBuffer : 버텍스 버퍼를 출력하기 위한 함수
	...
	// CreateIndexBuffer : 인덱스 버퍼를 출력하기 위한 함수
	if (FAILED(m_pGraphicDev->CreateIndexBuffer(m_dwIdxSize * m_dwTriCnt,	// 삼각형의 개수만큼 인덱스 버퍼를 잡는다
												0,							// 메모리 버퍼 사용방식 옵션, 0인 경우 정적 버퍼를 사용(D3DUSAGE_DYNAMIC을 삽입할 경우 동적 버퍼 용도로 변경)
												m_IdxFmt,					// 인덱스 속성 정보
												D3DPOOL_MANAGED,			// 메모리 저장 방식, 정적 버퍼인 경우엔 MANAGED고정
