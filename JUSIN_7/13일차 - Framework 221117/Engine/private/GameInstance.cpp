...
HRESULT CGameInstance::Initialize_Engine(HINSTANCE hInst, _uint iNumLevels, const GRAPHIC_DESC& GraphicDesc, ID3D11Device** ppDeviceOut, ID3D11DeviceContext** ppContextOut)
{
	...
	// CGameInstance::Initialize_Engine()
	if (FAILED(m_pComponent_Manager->Add_Prototype(m_iStaticLevelIndex/*LEVEL_STATIC*/, 
		m_pPrototypeTransformTag/*TEXT("Prototype_Component_Transform")*/, 
...
_matrix CGameInstance::Get_TransformMatrix(CPipeLine::TRANSFORMSTATE eState)
{
	if (nullptr == m_pPipeLine)
		return XMMatrixIdentity(); // nullptr 이라면 항등행렬 던져주기
...
{
	if (nullptr == m_pPipeLine)
		return _float4x4(); // nullptr 이라면 쓰레기값 던져주기
...
_float4 CGameInstance::Get_CamPosition()
{
	if (nullptr == m_pPipeLine)
	{
		MSG_BOX("PipeLine nullptr");
		return _float4(); // nullptr 이라면 쓰레기값 던짐
	}
