...
void CPlayer::Key_Input(void)
{
	...
	if (GetAsyncKeyState('W'))
	{
		...
		if (GetAsyncKeyState('D')) // 오른쪽 대각선
		{
			D3DXVec3TransformNormal(&m_tInfo.vDir, &m_tInfo.vLook, &m_tInfo.matWorld);
			m_tInfo.vPos.x += sinf(45) * (m_tInfo.vDir.x * 15.f);
			m_tInfo.vPos.y -= cosf(45) * (m_tInfo.vDir.y * 15.f);
			// m_tInfo.vLook = D3DXVECTOR3(1.f, -1.f, 0.f); 이렇게 초기화 되어있어서
			// 반대로 sinf, cosf반대로
			// 스케일을 적용해서 그런지 대각선 이동시 속도도 그렇고, 뭔가 부자연스럽다
		}
	}
