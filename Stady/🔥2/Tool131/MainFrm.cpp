...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	//	CreateStatic : 분할된 창을 만들기 위한 함수
	//	CreateStatic(부모 윈도우 주소, 행의 수, 열의 수, 창 스타일 옵션, 자식 창 id 값)

	/*m_MainSplitter.CreateStatic(this, 1, 2);

	//CreateView: 분할 창에 표시한 view창을 생성, 지정하는 함수
	//CreateView(배치할 행, 배치할 열, 배치할 창 생성 매크로, 초기 크기, pContext);

	m_MainSplitter.CreateView(0, 0, RUNTIME_CLASS(CMyForm), CSize(300, WINCY), pContext);
	m_MainSplitter.CreateView(0, 1, RUNTIME_CLASS(CTool131View), CSize(WINCX, WINCY), pContext);*/
	...
	m_MainSplitter.CreateView(1, 1, RUNTIME_CLASS(CTool131View), CSize(WINCX, WINCY), pContext);*/
	...
	// WS_CHILD: 자식 창의 형태, WS_VISIBLE : 생성 후 바로 화면에 표시하겠다는 옵션
