...
{
	if (fabs(pDest->Get_CollisionInfo().vPos.x - pSour->Get_CollisionInfo().vPos.x) >
		pDest->Get_Radius() + pSour->Get_Radius())
		return false;

	if (fabs(pDest->Get_CollisionInfo().vPos.y - pSour->Get_CollisionInfo().vPos.y) >
		pDest->Get_Radius() + pSour->Get_Radius())
		return false;
...
{
	D3DXVECTOR3 vPos = pDest->Get_CollisionInfo().vPos - pSour->Get_CollisionInfo().vPos;
