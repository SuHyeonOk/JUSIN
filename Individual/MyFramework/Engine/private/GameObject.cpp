...
HRESULT CGameObject::Initialize(void * pArg)
{
	...
	{
		if (FAILED(Add_Component(CGameInstance::Get_StaticLevelIndex(), CGameInstance::m_pPrototypeTransformTag, 
			m_pTransformComTag,					// TEXT("Com_Transform")
			(CComponent**)&m_pTransformCom,		// 상속 받은 객체에서 멤버변수로 사용할 수 있도록 주소를 넘긴다.
			&GameObjectDesc.TransformDesc)))	// pArg 를 넘김 (현재는 Transform 컴포넌트만 있음)
			return E_FAIL;
	}
