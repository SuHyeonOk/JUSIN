...
void CFrustum::Transform_ToWorldSpace()
{
	...
	서로 다르기때문에 한번 변환시킨다라는 개념이 사용 불가능하다. 각개체안에서 로컬로 변환시켜 정점위치와 위치판단하겠다.  */
	...
	// 투영 스페이스에 있는 절두체를 뷰, 투영의 역행렬을 곱해서 월드 스페이스로 변경한다.
	for (_uint i = 0; i < 8; ++i)
	{
...
{
	// 월드 스페이스에 존재한 절두체를 투영 스페이스 까지 내리기 위한 작업
...
_bool CFrustum::isInFrustum_WorldSpace(_fvector vWorldPos, _float fRange)
{
	for (_uint i = 0; i < 6; ++i)
	{
		if (fRange < XMVectorGetX(XMPlaneDotCoord(XMLoadFloat4(&m_PlanesInWorld[i]), vWorldPos)))
			return false;	// 양수 : 절두체 밖 에 있다.
	}

	return true;			// 음수 : 절두체 안 에 있다
}
