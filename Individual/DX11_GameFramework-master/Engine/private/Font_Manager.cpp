...
{
	if (nullptr != Find_Font(pFontTag))	// 중복된 폰트가 들어가지 않도록 예외처리
		return E_FAIL;

	CCustomFont*		pFont = CCustomFont::Create(pDevice, pContext, pFontFilePath);	// 폰트 생성
	if (nullptr == pFont)
		return E_FAIL;

	m_Fonts.emplace(pFontTag, pFont); // 맵에 담기
